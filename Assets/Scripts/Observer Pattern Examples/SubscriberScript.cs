...
public class SubscriberScript : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        publisher.OnSpacePressed += OnSpacePressedSubscriber;
    }

    // Whenever the signal is sent, the subscriber can use any information carried within.
    private void OnSpacePressedSubscriber(object sender, 
        PublisherScript.OnSpacePressedEventArgs e)
    {
        // The visual queue is a delayed jump based on position.
        StartCoroutine(Jump(DelayTime(e.position)));
    }
